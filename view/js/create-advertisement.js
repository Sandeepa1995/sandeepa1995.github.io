...
var socket=io('/create-advertisement');
...
function genText(){
    document.getElementById("typeText").innerHTML="Make a General Advertisement";
}

function spcText(){
    document.getElementById("typeText").innerHTML="Make a Specific Advertisement";
}
...
function getCreator() {
    socket.emit('getNoticeCreator',{
        ...
        thisuser=user;
        var position = user.name+'\n'+user.type +' - '+user.batch[0].department +' '+ user.batch[0].year;
...
function setTitle() {
    var position = thisuser.name + '\n' + thisuser.type + ' - ' + thisuser.batch[0].department + ' ' + thisuser.batch[0].year;
...
function loadList(list) {
    for (var indx = 0; indx < list.users.length; ++indx) {
        ...
        if (userDetails.iD==loggedID){
            var toADD = "<tr onclick='selectRow(this,"+checkNum+")' data-toggle='modal' data-target='#displayModal' style='cursor: pointer'><td><input onclick='flipCheckItself(this,"+checkNum+")' type='checkbox' name='optionsCheckboxes' class='checkbox'></td><td>"+userDetails.iD+"</td><td>"+userDetails.name+"</td> <td>"+userDetails.batch[0].department+"</td><td>"+userDetails.batch[0].year+"</td><td>"+userDetails.type+"</td></tr>";
        }
        else {
            targetgrp.push(userDetails.iD);
            var toADD = "<tr onclick='selectRow(this," + checkNum + ")' data-toggle='modal' data-target='#displayModal' style='cursor: pointer'><td><input onclick='flipCheckItself(this," + checkNum + ")' type='checkbox' name='optionsCheckboxes' class='checkbox' checked='checked'></td><td>" + userDetails.iD + "</td><td>" + userDetails.name + "</td> <td>" + userDetails.batch[0].department + "</td><td>" + userDetails.batch[0].year + "</td><td>" + userDetails.type + "</td></tr>";
        }
        checkNum+=1;
